...
function random_adj (lenght){
  let randomNumber = 0 + Math.random() * (lenght - 0);
  let i = parseFloat(randomNumber.toFixed(0));
  let word = adj[i];
  return word;
}
function random_noun (lenght){
  let randomNumber = 0 + Math.random() * (lenght - 0);
  let i = parseFloat(randomNumber.toFixed(0));
  let word = noun[i];
  return word;
...
app.post("/submit", (req, res) => {
  ...
  //Then:
  let adj = random_adj(1346);
  let noun = random_noun(4300);
  console.log(noun);
  ...
  res.render("index.ejs", {adjective: adj, Noun: noun});
